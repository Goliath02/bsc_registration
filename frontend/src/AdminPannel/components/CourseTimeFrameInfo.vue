	...
	<div class="flex flex-col gap-2 text-center">
		<div class="flex gap-8 justify-around items-center">
			...
			<div class="flex flex-col justify-center items-center font-bold">
				...
				<div v-if="dates.length > 0">{{ formatDate(dates[dates.length - 1]) }}</div>
			</div>
		</div>
		<div>
			<Message v-for="holiday in holidayDates" v-if="holidayDates.length > 0" severity="error">
				...
				{{ holiday.holidayName }}
						...
						from
						<span class="font-bold">{{ formatDate(holiday.fromDate) }}</span> - <span
							class="font-bold">{{ formatDate(holiday.fromDate) }}</span>
					</div>
				</div>
			</Message>
		</div>
