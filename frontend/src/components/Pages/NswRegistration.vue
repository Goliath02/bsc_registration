...
<template>
  ...
  >
    <Panel header="Info zum Nichtschwimmerkurs" class="bg-bsc-lightgray!">
      <p class="font-bold">
        Bitte melden Sie sich nur an wenn, man Sie von einer den Kurs-Leiter
        aufgefordert hat!
      </p>

      <br />

      Neben den Kosten des Nichtschwimmer-Kurses müssen Sie auch ein
      Mitgliederbeitrag bezahlen.
      <br />
      Nach dem Nichtschwimmerkurs wird der Mitgliederbeitrag weitergeführt und
      sind weiterhin Mitglied.
      <br />
      Wenn das nicht gewollt ist, Kündigen Sie bitte fristgerecht, wie in der
      Anmeldung angegeben.
    </Panel>
    ...
    <div class="flex gap-[1.5em]">
      <BSCInput
        ...
        :is-not-valid="
          ...
          useRegistrationStore().triedToValidateBasicForm
        "
    ...
    <div class="flex gap-[1.5em] max-[640px]:flex-col">
      ...
      <BSCInput
        v-model.modelValue="
          useRegistrationStore().registrationData.mainData.entryDate
        "
        :is-not-valid="false"
    ...
    <ExtraPersonForm
      v-for="(person, index) in useRegistrationStore().registrationData
        .morePersons"
      v-model.extraModelValues="
        useRegistrationStore().registrationData.morePersons[index]
      "
      :index="index"
      :input-data="{
        name: person.name,
