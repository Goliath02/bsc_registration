...
const resolver = yupResolver(yup.object().shape({
    ...
    "file": yup
	    .mixed().test("isValidUpload", "Bitte Datei hochladen.", (value) => isvalidUpload(value)),
  "morePersons": yup.array().of(
      yup.object({
        ...
        extraBirthday: yup.date().required('Geburtstag ist erforderlich').typeError('Ungültiges Datum'),
...
const isvalidUpload = () => {
	...
	return upload.length > 0 || useRegistrationStore().registrationData.mainData.type !== RegistrationType.STUDENT;
};
...
<template>
	...
	<FormHeader header-text="Mitgliederregistrierung"/>
      ...
      <div v-if="isFamilyRegistration" v-for="(person, index) in useRegistrationStore().registrationData.mainData.morePersons" :key="index">
        <ExtraPersonForm
            v-model="useRegistrationStore().registrationData.mainData.morePersons[index]"
            :index="index"
        />
      </div>
