<script>
import Header from "@/components/BSC_Header.vue";

export default {
  name: "RequestSuccessPage",
  components: { Header },
};
</script>

<template>
  <Header />
  <main
    class="flex flex-1 justify-center items-center lg:px-[7%] lg:py-[2%] min-h-0 bg-[url('@/assets//Water.jpg')] bg-cover overflow-y-auto"
  >
    <div
      class="lg:max-w-[50%] overflow-y-auto h-full bg-bsc-gray rounded-lg flex flex-col min-h-0 text-white text-center gap-[2em] -lg:px-[1em] lg:px-[6em] pt-[3em] pb-[2em]"
    >
      <img
        class="h-[10em] px-[2em]"
        src="@/assets/icons/Full%20BSC%20Logo.svg"
      />

      <h1 class="text-4xl font-bold">Vielen Dank für Ihr Interesse!</h1>

      <div class="flex flex-col justify-center text-xl font-bold gap-[0.7em]">
        <p>Wir werden ihre Daten so schnell wie möglich bearbeiten</p>
        <p>Weitere Informationen erhalten Sie in der Bestätigungs-email</p>
        <p>
          Wenn Sie weitere Fragen haben kontaktieren sie
          <a
            class="text-blue-500"
            href="mailto: vorstand@erster-bsc-pforzheim.de"
            >vorstand@erster-bsc-pforzheim.de</a
          >
        </p>

        <a href="/">
          <button
            class="border-white border font-bold px-[1em] py-[0.5em] rounded"
          >
            Zurück
          </button>
        </a>
      </div>
    </div>
  </main>
</template>

<style scoped></style>
