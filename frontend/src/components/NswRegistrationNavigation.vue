  ...
  methods: {
    ...
    postData() {
      ...
      if (!useRegistrationStore().registrationData.hiddenSecurityCheck) {
        ...
        axios.post(useRegistrationStore().getTargetURL() + "/registrateNsw", formData, {headers: {'Content-Type': 'multipart/form-data'}})
...
<template>
  ...
  <div class="flex p-[1em]">
    ...
    <div class="basis-1/3 flex justify-end">
      <button v-if="!isLastPage" class="font-bold bg-red-600 px-[1em] py-[0.5em] rounded cursor-pointer" @click="routerToNextSite">
        {{ getLastPageButtonText }}
      </button>
